...
# transcripts to read. Meta first, so the join is already built when the data arrives.
{
  ...
  BEGIN {
    ...
    while ((getline line < EV) > 0) {
      ...
      # pack_loads joins the key so a transcript recorded before that field existed is
      # rewritten ONCE and then left alone: an old row reads 0 here, and a session that
      # loaded no pack computes 0 too, so only the treated sessions move. Without it the
      # field could never reach the rows already recorded, and the population that would
      # answer the question is exactly the one already on disk.
      seen[k] = num(line, "tok_in") "|" num(line, "tok_out") "|" \
                num(line, "cache_read") "|" num(line, "cache_write") "|" num(line, "pack_loads")
    }
    close(EV)
  }
  ...
  $1 == "F" {
    key = $2; path = $3
    tin = tout = tcr = tcw = ctx = 0; model = ""; turns = 0; packs = 0
    while ((getline line < path) > 0) {
      # Did this session load a cluster pack? Counted BEFORE the usage filter, because a pack
      # arrives in a tool result rather than in an assistant record carrying usage.
      #
      # DERIVED, never self-reported. The claim that packs lower context per turn cannot be
      # tested without separating the sessions that loaded one, and a flag the session sets
      # about itself is the signature that carries no information -- the same rule as `Via:`.
      # The transcript already contains the answer verbatim, so it is read rather than asked for.
      #
      # The pattern matches the RENDERED header and not the generator that prints it. The naive
      # form matched the generator line inside kit-plan.sh, so a session that merely READ it
      # scored as a pack consumer: [^%] before the comma and a digit after `cluster` are what
      # separate a rendered "cluster 4)" from the literal "cluster %s)" in the format
      # string. Measured on the transcripts here, that moved three sessions off the arm.
      if (line ~ /GENERATED by kit-plan\.sh \(goal [^%]*, cluster [0-9]+\)/) packs++
    ...
    if (turns == 0) next
    if (seen[key] == tin "|" tout "|" tcr "|" tcw "|" packs) next
    ...
    printf "{\"task\":\"\",\"kind\":\"spend\",\"at\":\"%s\",\"transcript\":\"%s\",\"scope\":\"%s\",\"agent\":\"%s\",\"agent_id\":\"%s\",\"session\":\"%s\",\"model\":\"%s\",\"turns\":%d,\"tok_in\":%d,\"tok_out\":%d,\"cache_read\":%d,\"cache_write\":%d,\"context\":%d,\"pack_loads\":%d}\n",
