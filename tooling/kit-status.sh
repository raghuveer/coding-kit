...
# FINDING ATTRIBUTION. A finding names the ROLE that produced it and, since 2026-09-14, the RUN.
# Without the run it cannot be joined to that reviewer's spend row, so "what did this reviewer
# cost and what did it find" -- one question about one run, and one of the two readings a trial
# takes -- is unanswerable. Measured on the 2026-09-09 trial: 11 findings, 0 attributable, while
# all 3 reviewer spend rows carried an id.
#
# TWO COUNTS, NOT ONE, because they are different faults with different fixes and a single
# "unattributed" number would hide the second entirely:
#   no id        the recorder did not pass --agent-id. Fix the caller.
#   id, no match the id was passed and matches NO spend row. That is not a missing label, it is
...
_FORPH=$(q "SELECT COUNT(*) FROM finding f WHERE COALESCE(f.agent_id,'')<>''
              AND NOT EXISTS (SELECT 1 FROM spend s WHERE s.agent_id = f.agent_id);")
if [ "${_FNONE:-0}" != 0 ] || [ "${_FORPH:-0}" != 0 ]; then
  printf '\n> **Findings that cannot be joined to a reviewer run:** %s of %s carry no agent id' \
    "$_FNONE" "$_FN"
  [ "${_FORPH:-0}" = 0 ] || printf ', and %s carry one that matches no spend row' "$_FORPH"
  printf '.\n'
  printf '> An empty id is not an attributed finding. Pass `--agent-id` when recording; see\n'
  printf '> `skills/verify-ladder/SKILL.md`.\n\n'
fi

printf '## Goals\n\n'
# A MILESTONE'S STATE WAS INVISIBLE HERE, which made `#goal_state` write-only from the operator's
# side: the plan file carried it, the index derived it, and nothing showed it. Every figure below
# this line is implicitly `default` -- reporting fully per goal is AC2 of
# T-20260819-goals-are-the-milestone-mechanism-and-on and is NOT this; this is the one line that
# stops a second goal being invisible while that is built.
#
# The closed test is a join against state_class, never a literal: a goal state is a task state,
# normalised through the same alias table, so `completed` and a legacy `done` read identically.
GOALS=$(q "SELECT g.id||'  ['||g.state||']'||
             CASE WHEN g.state IN (SELECT state FROM state_class WHERE is_closed = 1)
                  THEN '  closed' ELSE '' END||
             '  '||(SELECT COUNT(*) FROM plan_item p WHERE p.goal_id = g.id)||' planned'
           FROM goal g ORDER BY g.id;")
[ -n "$GOALS" ] && printf '%s\n' "$GOALS" | sed 's/^/- /' || printf '_none_\n'
printf '\n'

printf '## Open\n\n'
# PARKED IS SAID OUT LOUD, with what waits on it. `on-hold` was rendered indistinguishably from
# `created` here, so the only visible difference parking made was a word in a sort key -- and the
# planner could not see it either, which is the defect this pairs with. The count matters because
# parking one task withholds everything behind it, and that tail is invisible from the task itself.
OPEN=$(q "SELECT t.id||'  '||COALESCE(NULLIF(n.title,''),t.id)||'  ['||COALESCE(NULLIF(t.tier,''),'untiered')||']'||CASE WHEN t.state IN (SELECT state FROM state_class WHERE is_plannable = 0 AND is_closed = 0) THEN '  **parked**, '||(SELECT COUNT(*) FROM edge e WHERE e.rel='depends_on' AND e.dst=t.id)||' waiting' ELSE '' END||CASE WHEN t.blocked_by<>'' THEN '  blocked-by '||t.blocked_by ELSE '' END||CASE WHEN COALESCE(t.owner,'')<>'' THEN '  @'||t.owner ELSE '' END
